        ...
        processes.update(
            {
                ...
                + str(i + 1): {
                    ...
                    "priority": int(
                        input("Enter priority of process " + str(i + 1) + ": ")
                    ),
                }
            }
        )
...
def updateData():
    for i in range(pcount):
        ...
        processes[process]["turnAroundTime"] = (
            ganttChart[i]["endTime"] - processes[process]["arrivalTime"]
        )
        processes[process]["waitingTime"] = (
            processes[process]["turnAroundTime"] - processes[process]["burstTime"]
        )
