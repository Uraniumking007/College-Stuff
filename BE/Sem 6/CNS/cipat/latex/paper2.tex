...
\begin{table}[!t]
\caption{Standard vs.\ Optimized RSA Performance (2048-bit, 50 Samples)}
...
\hline
Encrypt & 55.556 & 0.026 & $2136\times$ \\
\hline
Decrypt & 57.636 & 0.771 & $75\times$ \\
\hline
Enc Memory (KB) & 0.9 & 0.6 & $1.5\times$ \\
\hline
Dec Memory (KB) & 0.9 & 0.8 & $1.1\times$ \\
\hline
\end{tabular}
\end{table}

The dramatic speedup is primarily attributable to key caching in the optimized implementation. The standard implementation generates fresh keys per operation (cold start), while the optimized variant reuses cached keys. The encryption speedup of $2136\times$ reflects the elimination of the 2048-bit key generation bottleneck (which dominates the standard implementation's 55.5~ms encrypt time). Decryption benefits from both key caching and CRT-based computation, yielding a $75\times$ improvement.
...
\begin{table}[!t]
\caption{Key-Size Scaling Performance (Optimized RSA, 30 Samples)}
\label{tab:keysize}
\centering
...
\hline
1024-bit & 0.014 & 0.131 & 0.49 & 0.41 \\
\hline
2048-bit & 0.025 & 0.740 & 0.59 & 0.53 \\
\hline
3072-bit & 0.043 & 1.820 & 0.72 & 0.66 \\
\hline
4096-bit & 0.069 & 3.585 & 0.84 & 0.78 \\
\hline
\end{tabular}
\end{table}

Encryption time grows modestly from 0.014~ms (1024-bit) to 0.069~ms (4096-bit), a $4.9\times$ increase for a $4\times$ key-size increase. This sub-quadratic growth occurs because the public exponent $e = 65537$ remains fixed at 17~bits regardless of key size. Decryption time grows more steeply from 0.131~ms to 3.585~ms, a $27.4\times$ increase, consistent with the $O(n^2 \log n)$ complexity of modular exponentiation.

The growth factors per doubling are:
\begin{itemize}
    \item 1024 $\rightarrow$ 2048: encrypt $1.8\times$, decrypt $5.6\times$
    \item 2048 $\rightarrow$ 3072: encrypt $1.7\times$, decrypt $2.5\times$
    \item 3072 $\rightarrow$ 4096: encrypt $1.6\times$, decrypt $2.0\times$
\end{itemize}
...
\hline
1~KB & 0.14 & 0.11 & $1.3\times$ \\
\hline
10~KB & 1.30 & 0.04 & $33\times$ \\
\hline
50~KB & 6.50 & 0.04 & $163\times$ \\
\hline
100~KB & 12.97 & 0.05 & $260\times$ \\
\hline
500~KB & 64.88 & 0.10 & $649\times$ \\
\hline
1000~KB & 129.18 & 0.17 & $760\times$ \\
\hline
\end{tabular}
\end{table}

Pure RSA encrypts data in 190-byte chunks, requiring multiple RSA operations proportional to data size. The hybrid approach performs exactly one RSA operation (to wrap the 32-byte AES key) regardless of data size, with AES-GCM handling bulk encryption at near-constant time. At 1~MB, the hybrid approach is $760\times$ faster. The hybrid encryption time grows from 0.04~ms at 10~KB to only 0.17~ms at 1~MB, confirming that AES-GCM throughput dominates.
...
\begin{table}[!t]
\caption{Concurrent Session Throughput (Optimized RSA, 2048-bit)}
...
The optimized implementation maintains stable throughput of approximately 1236--1294~sessions/second across all concurrency levels, with no degradation or errors. Average latency remains under 2~ms even at 50~concurrent threads. The near-constant throughput confirms that the key caching strategy eliminates the primary bottleneck under concurrent load.
...
\begin{table}[!t]
\caption{Summary: Theoretical vs.\ Experimental Speedup}
\label{tab:summary}
\centering
\begin{tabular}{|c|c|c|}
\hline
...
\hline
CRT Decryption & $3$--$4\times$ & $1.2\times$ (2048-bit) \\
\hline
Montgomery Mult. & $2$--$3\times$ & $0.3\times$ (Python) \\
\hline
Sliding-Window & $30$--$40\%$ fewer mult. & Marginal in Python \\
\hline
Key Caching & $N/A$ (deployment) & $75$--$2136\times$ \\
\hline
Hybrid RSA+AES & $8$--$400\times$ & $1.3$--$760\times$ \\
\hline
\end{tabular}
\end{table}

The discrepancy between theoretical and observed performance for Montgomery multiplication and sliding-window exponentiation is attributable to Python's interpreter overhead: per-operation costs (object creation, function dispatch, garbage collection) dominate the actual arithmetic for moderate key sizes. Key caching and hybrid encryption, which reduce the \textit{number} of expensive operations rather than per-operation cost, deliver the largest practical improvements.
...
\begin{enumerate}
    \item \textbf{Key caching is the highest-impact optimization}, providing $75$--$2136\times$ speedup by eliminating repeated key generation. This validates the importance of key management infrastructure in web server deployments.

    \item \textbf{Hybrid RSA+AES-GCM is essential for bulk data}, outperforming pure RSA by $1.3\times$ to $760\times$ across 1~KB to 1~MB data sizes. RSA should never be used alone for data exceeding a few hundred bytes.

    \item \textbf{CRT provides measurable decryption improvement} ($1.2\times$ at 2048-bit), though the theoretical $4\times$ prediction assumes compiled-language execution. In interpreted environments, the gain is modest.

    \item \textbf{Montgomery multiplication and sliding-window exponentiation underperform in Python} due to interpreter overhead. These techniques are better suited to C, Rust, or hardware implementations.

    \item \textbf{RSA scales approximately quadratically with key size}, making the transition to 3072-bit or 4096-bit keys (recommended by NIST~\cite{nist_sp80057}) a significant performance concern that must be mitigated through the optimizations surveyed.

    \item \textbf{Concurrent throughput is stable}, with the optimized implementation maintaining $>1200$~sessions/second across 1--50 threads, confirming suitability for multi-user web application scenarios.
\end{enumerate}
