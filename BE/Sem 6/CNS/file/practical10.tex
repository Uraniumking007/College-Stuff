...
% Justification for paragraphs
\justifying

\begin{document}

\section*{Practical No.10}
...
\begin{itemize}
    \item For a prime $p$: $\varphi(p) = p - 1$
    \item For distinct primes $p, q$: $\varphi(p \times q) = (p-1)(q-1)$
    \item General formula: $\varphi(n) = n \times \prod(1 - 1/p)$ for all distinct prime factors $p$ of $n$
\end{itemize}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{practical10-output.png}
    \caption{Output of Euler's Totient Function Implementation}
\end{figure}
