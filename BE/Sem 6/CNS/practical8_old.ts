...
interface RSAKeyPairParameters {
  p: bigint
  q: bigint
  e?: bigint // Optional: if not provided, will be generated
}
 ...
 */
function generateRSAKeyPair(parameters: RSAKeyPairParameters): RSAKeyPair {
  ...
  if (providedE) {
    ...
    if (providedE <= 1n || providedE >= phi) {
      throw new Error(
        `e must satisfy: 1 < e < φ(n) (got e=${providedE}, φ(n)=${phi})`
      )
    }

    if (gcd(providedE, phi) !== 1n) {
      throw new Error(
        `e must be coprime with φ(n) (gcd(${providedE}, ${phi}) ≠ 1)`
      )
    }
...
function rsaEncrypt(M: bigint, publicKey: { n: bigint; e: bigint }): bigint {
  if (M >= publicKey.n) {
    throw new Error(
      `Message must be less than modulus (M=${M}, n=${publicKey.n})`
    )
  }
...
function rsaDecrypt(C: bigint, privateKey: { n: bigint; d: bigint }): bigint {
  if (C >= privateKey.n) {
    throw new Error(
      `Ciphertext must be less than modulus (C=${C}, n=${privateKey.n})`
    )
  }
...
testMessage.split('').forEach((char, i) => {
  console.log(
    `  '${char}' (ASCII ${char.charCodeAt(0)}) → ${encryptedChars[i]}`
  )
})
